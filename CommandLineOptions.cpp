...
namespace Utilities {
  ...
  : optChar(_optChar),
    optType(_optType),
    longOpt(_longOpt),
    optDescription(_optDescription)
  {
  }
  ...
  : optChar(c.optChar),
    optType(c.optType),
    longOpt(c.longOpt),
    optDescription(c.optDescription)
  {
  }
  ...
  void CommandLineOptions::FreeOptionDefinitions()
  {
    if (optionDefinitions != 0) {
      int i= 0;
      while (true) {
        if (optionDefinitions[i] == 0) break;
  ...
  void CommandLineOptions::FreeOptionDescriptions()
  {
    if (optionDescriptions != 0) {
      int i= 0;
      while (true) {
        if (optionDescriptions[i] == 0) break;
  ...
  void CommandLineOptions::SetPositional(
    int minNum, int maxNum, std::vector<std::string>* params)
  {
  ...
  CommandLineOptions::OptionReturnCode
    CommandLineOptions::ProcessOptions(int argc, const char* argv[])
  {
